...
class ServiceInfo(object):
    ...
    def __str__(self):
        return "<ServiceInfo: {}:{}.{}>".format(self.service_name,
                                                self.major_version,
                                                self.minor_version)
...
def _decode_response(stream):
    ...
    while offset < len(json_string):
        (chunk, new_offset) = decoder.raw_decode(
            json_string, idx=offset)
...
class HttpServiceClient(object):
    ...
    def _make_bas_request(self, request, user_context):
        ...
        service_version = "{}.{}".format(
            self._service_info.major_version,
            self._service_info.minor_version)
        user_info = [
    ...
    def _send_request_async(self,
        ...
        fut = self._executor.submit(self._send_request_sync,
                                    request,
                                    user_context,
                                    timeout)
        futex = FutureExt(fut)
    ...
    def send_request(self,
        ...
        return (
            self._send_request_sync(request, user_context, timeout) if is_sync
            else self._send_request_async(request,
                                          user_context,
                                          callback,
                                          error_callback,
                                          timeout))
