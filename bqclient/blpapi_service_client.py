...
AsyncRequest = namedtuple(
    "AsyncRequest",
    ["operation", "request", "callback", "error_callback", "promise"],
)


class BlpapiServiceClient:
    def __init__(
        self, service_name, max_concurrent_requests=10, bqapi_session=None
    ):
    ...
    def _execute_send_request(
        self, operation, request, callback=None, error_callback=None
    ):
    ...
    def _trigger_waiting_requests(self):
        ...
        while self._wait_queue and not self.at_max_concurrent_requests:
            ...
            ).then(set_promise_result, error_handler)

    def _send_request_async(
        self, operation, request, callback, error_callback=None
    ):
        ...
        if self.at_max_concurrent_requests:
            promise = bqapi.Promise(self._session.event_loop)
        ...
        def cb_wrapper(message):
            try:
                ...
                _logger.debug(
                    "Received %d responses: %s", len(responses), responses
                )
            except Exception:
                if error_callback is not None:
        ...
        self._num_pending_requests += 1
        return self._execute_send_request(
            operation, request, cb_wrapper, error_cb_wrapper
        )
    ...
    def send_request(
        ...
        if callback is not None:
            return self._send_request_async(
                operation, request, callback, error_callback
            )

        # synchronous request
        msg = self._execute_send_request(operation, request)
        ret = [resp.element() for resp in msg]
        return ret
